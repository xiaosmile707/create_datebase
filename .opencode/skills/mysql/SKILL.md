---
name: mysql
description: SSH 到目标机器，通过 Docker 创建 MySQL 容器，将连接信息写入 db-connection.json
---
...
### 2. 确认参数
向用户展示默认参数，允许修改：版本、端口、容器名、root 密码、内存限制。
...
### 6. 健康检查
循环检查 MySQL 是否就绪，最多等待 60 秒：
```
docker exec <容器名> mysqladmin ping -h localhost -u root -p<root密码>
```
成功后输出 "MySQL is ready"。

### 7. 写入 db-connection.json
将连接信息写入 `db-mysql/src/main/resources/db-connection.json`：
```json
{
  "host": "<ssh.host>",
  "port": <映射端口>,
